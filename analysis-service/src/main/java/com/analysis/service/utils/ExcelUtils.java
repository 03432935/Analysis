...
public class ExcelUtils {
    ...
    // 这里每次会读取3000条数据 然后返回过来 直接调用使用数据就行
        EasyExcel.read(fileName, ImportDto.class, new PageReadListener<ImportDto>(dataList -> {
            for (ImportDto demoData : dataList) {
    ...
    public static void read(String fileName, ImportExcelService importExcelService) {
        ...
        try {
            excelReader = EasyExcel.read(fileName, ImportDto.class, new ImportExcelListener(importExcelService)).build();
    ...
    public static void readWithListener(String fileName, ImportExcelService importExcelService){
        ...
        // 写法3：
        // 这里 需要指定读用哪个class去读，然后读取第一个sheet 文件流会自动关闭
        EasyExcel.read(fileName, ImportDto.class, new ImportExcelListener(importExcelService)).sheet().doRead();
    }

    public static void readWithInputStream(InputStream inputStream, ImportExcelService importExcelService){

        EasyExcel.read(inputStream, ImportDto.class, new ImportExcelListener(importExcelService)).sheet().doRead();
    }
}

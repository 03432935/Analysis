...
public class ARMAMethod
{
	...
	/**
	 * 计算自相关函数(系数) rou(k) = C(k) / C(0); 其中 C(k) = sum((x(t) - mu)*(x(t - k) -
	 * mu)) / (N - k), C(0) = var = sum(x(t) - mu) ^2 / N;
	 * 
	 * @param originalData
	 * @param order
	 * @return 自相关函数(rou(k))
	...
	public double getModelAIC(Vector<double[]> vec, double[] data, int type)
	{
		...
		else
		{
			...
			// return
			// (n-(p-1))*Math.log(sumErr/(n-(p-1)))+(p+q-1)*Math.log(n-(p-1));
			// //AIC 最小二乘估计
		}
	}
	...
	// * @param garma
	// * 代表的是数据的协方差
	// * @return 返回经由Y-W方程求解的结果，其中数组的最后 一个元素存储的是模型中的噪声方差
	// */
	...
	// /*
	// * 可能会存在矩阵不可逆的情况，在矩阵不可逆时 可以通过将对角线元素全部增加1e-6做修正
	// */
	 ...
	 * @param garma
	 *            代表的是数据的协方差
	 * @return 返回结果的第一行元素代表的是在迭代过程中的方差， 其余的元素代表的是迭代过程中存储的系数
	 */
	...
	public double[] computeMACoe(double[] originalData, int q)
	{
		...
		System.out.println("computeMACoe " + len);
		for (int i = 1; i < len; ++i)
		{
